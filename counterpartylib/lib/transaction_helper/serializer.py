...
def make_p2sh_encoding_redeemscript(datachunk, n, pubKey):
    ...
    outputScript = [OP_HASH160, bitcoinlib.core.Hash160(datachunk), OP_EQUALVERIFY]

    redeemScript = list(outputScript)
    ...
    logger.debug('redeemScript %s (%s)' % (repr(redeemScript), binascii.hexlify(redeemScript)))

    logger.debug('datachunk %s -> %s' % (binascii.hexlify(datachunk), binascii.hexlify(CScript([datachunk]))))

    outputScript = redeemScript.to_p2sh_scriptPubKey()
    scriptSig = CScript([datachunk]) + redeemScript  # PUSH(datachunk) + redeemScript

    logger.debug('scriptSig %s (%s)' % (repr(scriptSig), binascii.hexlify(scriptSig)))
...
def serialise_p2sh_datatx(txid, source_script, destination_outputs, data_output, pubkey=None):
    assert data_output  # we don't do this unless there's data
    ...
    # list of inputs
    for data_idx, data_chunk in enumerate(data_array):
        data_chunk = config.PREFIX + data_chunk  # prefix the data_chunk
        ...
        # get the scripts
        scriptSig, redeemScript, outputScript = make_p2sh_encoding_redeemscript(data_chunk, data_idx, pubkey)

        s += txhash                             # TxOutHash
        s += (data_idx + 1).to_bytes(4, byteorder='little')  # TxOutIndex
        s += var_int(int(len(scriptSig)))       # Script length
        s += scriptSig                          # Script
        s += b'\xff' * 4                        # Sequence
