...
def construct_coin_selection(source, allow_unconfirmed_inputs, unspent_tx_hash, custom_inputs,
    ...
    if custom_inputs is None:
        ...
        for output in unspent:
            if make_outkey(output) not in filter_unspents_utxo_locks and make_outkey_vin(output['txhex'], output['vout']) not in filter_unspents_p2sh_locks:
...
def construct (db, tx_info, encoding='auto',
    ...
    if encoding == 'p2sh':
        ...
        size_for_fee = 10 + ((25 + 9) * len(destination_outputs)) + sum_data_output_size
    ...
    if encoding == 'p2sh':
        ...
        if pretx_txid:
            unsigned_datatx = serializer.serialise_p2sh_datatx(pretx_txid,
                                                               source_script=serializer.get_script(source_address),  # @TODO: p2sh?
