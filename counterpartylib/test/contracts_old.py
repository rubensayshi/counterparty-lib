...
from counterpartylib.lib.messages.ethereum import (blocks, processblock, utils)
import rlp
...
class tester(object):
    ...
    class state(object):
        ...
        def create_contract(self, code, endowment=0, sender=''):
            ...
            success, data = self.do_send(sender, '', endowment, data=code)
            contract_id = data
            return contract_id

        def evm(self, evmcode, endowment=0, sender=''):
            # Publish code.
            contract_id = self.create_contract(evmcode, endowment=endowment, sender=sender)
        ...
        def contract(self, code, sender='', endowment=0):
            ...
            return self.evm(evmcode, endowment=endowment, sender=sender)
        ...
        def do_send(self, sender, to, value, data=[]):
            ...
            tx_index += 1
            tx_obj = execute.Transaction(tx, 0, to, 1, tester.gas_limit, value, data)
            ...
            # @TODO
            tx_obj.nonce = block_obj.get_nonce(sender)
        ...
        def send(self, sender, to, value, data=[]):
            print('SEND', sender, to, value, data)
            ...
            success, output = self.do_send(sender, to, value, data=data)

            print('SEND-', success, output, utils.decode_datalist(bytes(output)))
            if output:
                return utils.decode_datalist(bytes(output))
            else:
                return []
